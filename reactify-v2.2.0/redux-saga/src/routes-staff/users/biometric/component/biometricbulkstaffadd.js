...
class BiometricBulkMemberAdd extends Component {
  ...
  onAdd(){
    let {data,biometricdevicelist}= this.props;
              ...
              api.post('BulkUploadUsersInBiometric',{biometricdata})
                .then(response =>
                 {
   ...
   const {data,biometricdevicelist} = this.props;
		return (
			<div>
             ...
             <Button color="primary" variant="contained" onClick={() => this.onclickAdd()}  className="mr-10 text-white">Add Users Into Biometric Device</Button>
					</div>
      <Dialog fullScreen = {isMobile ? true : false} fullWidth = {true} maxWidth = 'md'
        ...
        >
				<DialogTitle >
								{data &&		<span className="text-capitalize"> Total {data.length} Users to Add Into Biometric Device</span>}
				</DialogTitle>
...
export default connect(mapStateToProps,{push,getBiomaxUsers})(BiometricBulkMemberAdd);
