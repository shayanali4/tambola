var axios = require ('axios');

exports.bm_manuallyuserdeleted = function(req, res){
          const {client , user} = res.locals.oauth.token;
          let request_data =
                    {
                        "ApiRequestInfo": {
                                           "AuthToken": "COJJ7eiiPBGUfmIQPvh2PJWWDLX7OuKs",
                                            "Operation": "ManuallyUserDeleted",
                                            "OperationData": "",
                                            "OperationReferenceId": "",
                                            "OperationTime": "1560771790",
                                            "UserId": "2"
                                          },
                          "ServiceTagId": "ST-KY12312312"
                    }
             // axios.post(/url).then(response =>
             //     { writeLog({data :response ? response.data : {}});
    };
exports.bm_manuallyuserupdatedwithcard = function(req, res){
              const {client , user} = res.locals.oauth.token;
              let request_data =
                            {
                            "ApiRequestInfo": {
                                                 "AuthToken": "COJJ7eiiPBGUfmIQPvh2PJWWDLX7OuKs",
                                                 "Operation": "ManuallyUserUpdated",
                                                 "OperationData": {
                                                      "Name": "Suriya",
                                                      "Signature": [
                                                           {
                                                                "Template": "5634442",
                                                                "Type": "Card"
                                                           }
                                                      ],
                                                      "Priority": "User"
                                                 },
                                                 "OperationTime": "1554552359",
                                                 "UserId": "852"
                                            },
                                            "ServiceTagId": "ST-KY19123123"
                        }
             // axios.post(/url).then(response =>
             //     { writeLog({data :response ? response.data : {}});
    };
exports.bm_manuallyuserupdatedwithfingerprint = function(req, res){
                    const {client , user} = res.locals.oauth.token;
                    let request_data =
                          {
                              "ApiRequestInfo": {
                                                    "AuthToken": "COJJ7eiiPBGUfmIQPvh2PJWWDLX7OuKs",
                                                     "Operation": "ManuallyUserUpdated",
                                                     "OperationData": {
                                                          "Signature": [
                                                               {
                                                                    "AdditionalData1": "6",
                                                                    "Template": "",
                                                                    "Type": "Fingerprint"
                                                               }
                                                          ]
                                                     },
                                                     "OperationTime": "1554553164",
                                                     "UserId": "63"
                                                },
                                                "ServiceTagId": "ST-KY19123123"
                          }
                 // axios.post(/url).then(response =>
                 //     { writeLog({data :response ? response.data : {}});
        };
exports.bm_realtimepunchlog = function(req, res){
                const {client , user} = res.locals.oauth.token;
                            let request_data =
                                {
                                    "ApiRequestInfo": {
                                     "AuthToken": "COJJ7eiiPBGUfmIQPvh2PJWWDLX7OuKs",
                                      "Operation": "RealTimePunchLog",
                                      "OperationData": {
                                              "AttendanceType": "CheckIn",
                                              "InputType": "Card"
                                        },
                                       "OperationReferenceId": "",
                                       "OperationTime": "1557000413",
                                       "UserId": "55"
                                       },
                                    "ServiceTagId": "ST-KY19123123"
                              }
                     // axios.post(/url).then(response =>
                     //     { writeLog({data :response ? response.data : {}});
            };
