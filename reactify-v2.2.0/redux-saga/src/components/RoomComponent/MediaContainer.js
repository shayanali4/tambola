  ...
  render(){
    return (
      <div className={`media-bridge ${this.state.bridge}`}>
        <video className="remote-video" style = {this.props.facingMode  == "user" ? {transform: 'rotateY(180deg)'} : {transform: 'none'}} ref={(ref) => this.remoteVideo = ref} autoPlay></video>
        <video className="local-video" style = {this.props.facingMode  == "user" ? {transform: 'rotateY(180deg)'} : {transform: 'none'}} ref={(ref) => this.localVideo = ref} autoPlay muted></video>
      </div>
    );
  }
}
