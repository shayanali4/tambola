...
export default class ImageMapper extends Component {
  ...
  drawpolyManual(index) {

    let area  = this.props.map.areas[index];
    let coords = $("[name="+this.props.map.name+"]").children()[index].coords.split(',');
	...
	render() {
		return (
			<div style={this.styles.container} ref={node => this.container = node}>
				<img style={this.styles.img} src={this.props.src} useMap={`#${this.props.map.name}`} alt="" className = { this.props.className}
